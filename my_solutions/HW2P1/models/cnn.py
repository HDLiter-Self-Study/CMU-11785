...
class CNN(object):
    ...
    def __init__(
        ...
        self.output_widths = []
        # Fill convolutional_layers and output_widths in a loop
        current_input_width = input_width
        current_input_channels = num_input_channels
        for ind in range(len(num_channels)):
            out_channels = num_channels[ind]
            kernel_size = kernel_sizes[ind]
            stride = strides[ind]
            # Fill convolutional_layers
            conv1d = Conv1d(
            ...
            # Fill output_widths
            output_width = (current_input_width - kernel_size) // stride + 1
            self.output_widths.append(output_width)
            # Update variables
            current_input_width = output_width
            current_input_channels = out_channels

        self.flatten = Flatten()

        self.linear_layer = Linear(output_width * out_channels, num_linear_neurons)
    ...
    def forward(self, A):
        ...
        # Iterate through convs and activations
        for activation, conv_layer in zip(self.activations, self.convolutional_layers):
    ...
    def backward(self, labels):
        ...
        # Iterate through activations and convs
