    ...
    try:
        ...
        for arch_name in architectures:
            ...
            try:
                ...
                # Check blocks
                if arch_name == 'resnet':
                    se_blocks = [name for name, module in model.named_modules() 
                               if 'SEModule' in str(type(module))]
                    basic_blocks = [name for name, module in model.named_modules() 
                                  if 'BasicBlock' in str(type(module))]
                    bottleneck_blocks = [name for name, module in model.named_modules() 
                                       if 'BottleneckBlock' in str(type(module))]
                    ...
                    convnext_blocks = [name for name, module in model.named_modules() 
                                     if 'ConvNeXtBlock' in str(type(module))]
